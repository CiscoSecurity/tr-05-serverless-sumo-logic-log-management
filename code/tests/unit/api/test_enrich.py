...
from tests.unit.conftest import FORCE_PAUSED, CANCELLED, NOT_STARTED, GATHERING_RESULTS
...
def test_enrich_call_with_valid_jwt_but_invalid_json_value(
    mock_get, api_response, client, route, valid_jwt, invalid_json_value, invalid_json_expected_payload
):
...
@patch("requests.get")
def test_enrich_call_status_done(
    mock_get,
    mock_request,
    api_response,
    enrich_side_effect_list,
    client,
    route,
    valid_jwt,
    valid_json,
    ...
    mock_request.side_effect = [*enrich_side_effect_list(messages_count), *enrich_side_effect_list(1)]
    response = client.post(route, headers=get_headers(valid_jwt()), json=valid_json)
    assert response.status_code == HTTPStatus.OK
    assert response.json == expected_relay_response(messages_count=messages_count)
...
@patch("requests.request")
@patch("requests.get")
def test_enrich_call_not_started(
    mock_get,
    mock_request,
    api_response,
    ...
    mock_get.return_value = api_response(EXPECTED_RESPONSE_OF_JWKS_ENDPOINT)
    mock_request.return_value = api_response(general_response_payload_for_sumo_api_request(NOT_STARTED))
    response = client.post(route, headers=get_headers(valid_jwt()), json=valid_json)
    assert response.status_code == HTTPStatus.OK
...
def test_enrich_call_status_gathering_results(
    ...
    mock_request.return_value = api_response(
        general_response_payload_for_sumo_api_request(GATHERING_RESULTS, messages_count=99)
    )
    response = client.post(route, headers=get_headers(valid_jwt()), json=valid_json)
    assert response.status_code == HTTPStatus.OK
    assert response.json == expected_relay_response(state=GATHERING_RESULTS, messages_count=99)
...
def test_enrich_call_with_ssl_error(
    ...
    response = client.post("/observe/observables", headers=get_headers(valid_jwt()), json=valid_json)
    assert response.status_code == HTTPStatus.OK
...
def test_enrich_call_with_connection_error(
    mock_get, mock_request, api_response, client, valid_jwt, valid_json, connection_error_expected_relay_response
):
...
def test_enrich_call_with_bad_request_sumo_logic_error(
    mock_get, mock_request, api_response, client, valid_jwt, valid_json, bad_request_expected_relay_response
):

    mock_get.return_value = api_response(EXPECTED_RESPONSE_OF_JWKS_ENDPOINT)
    mock_request.return_value = api_response(text="Bad request to Sumo Logic", status_code=HTTPStatus.BAD_REQUEST)
