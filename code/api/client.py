...
class SumoLogicClient:
    ...
    @property
    def _headers(self):
        encoded = base64.b64encode(f'{self._credentials.get("accessId")}:{self._credentials.get("accessKey")}'.encode())
    ...
    def _request(
            self, path, method='GET', body=None, params=None, data_extractor=lambda r: r.json()
    ):
