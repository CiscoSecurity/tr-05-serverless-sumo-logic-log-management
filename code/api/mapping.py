...
DISPOSITION_MAP = {
    "high": {"disposition": 2, "disposition_name": "Malicious"},
    "medium": {"disposition": 2, "disposition_name": "Malicious"},
    "low": {"disposition": 3, "disposition_name": "Suspicious"},
    "unverified": {"disposition": 5, "disposition_name": "Unknown"},
}

SEVERITY_MAP = {"high": "High", "medium": "Medium", "low": "Low", "unverified": "Unknown"}
...
class Sighting:
    def _sighting(self, message, observable):
        sighting = {
            ...
            "external_ids": [message.get("_messageid")],
            "id": f'transient:{SIGHTING}-{uuid5(NAMESPACE_X500, message.get("_messageid"))}',
            "observables": [observable],
            "observed_time": {"start_time": self._start_time(message)},
    ...
    def sighting_source_uri(query, start_time, end_time):
        ...
        params = {"query": query, "startTime": start_time, "endTime": end_time}
    ...
    def _short_description(message):
        return (
            f'{message.get("_collector")} received a log from '
        ...
        return [
            {
                ...
                "source": {"type": "ip", "value": message.get("src_ip")},
                "related": {"type": "ip", "value": message.get("dest_ip")},
            }
        ]
...
class Judgement:
        ...
        judgement = {
            **DISPOSITION_MAP[cs_data["malicious_confidence"]],
            ...
            "valid_time": {
                ...
                "end_time": valid_time(cs_data["last_updated"], observable["type"]),
            },
    ...
    def _external_references(cs_data):
        ...
        for report in reports:
            references.append(
                {"source_name": "CrowdStrike", "description": "CrowdStrike Intelligence Report", "external_id": report}
            )
        return references

    @staticmethod
    def _transient_id(cs_data, observable):
        disposition = DISPOSITION_MAP[cs_data["malicious_confidence"]]["disposition"]
        seeds = f'{SOURCE}|{observable["value"]}|{disposition}|' f'{cs_data["last_updated"]}'
